...
public partial class Tesoreria_controlCajasVentas : System.Web.UI.Page
{
    ...
    {
        pnEstadoCierre.Visible = false;
        pnEstadoSuc.Visible = false;
    }

    #endregion


    protected void btnEstadoCierre_Click(object sender, EventArgs e)
    {
        string laccion;
        string ltipoSuc, lsucursal;
        DateTime lfechaInicio, lfechaFin;

        pnEstadoCierre.Visible = true;
        pnEstadoSuc.Visible = false;
        ...
        if (tipo == 4)
        {
            ...
            grvEstadoCierre.DataSource = dc.sp_ctrlCierres(laccion, ltipoSuc, lsucursal, lfechaInicio, lfechaFin);
            grvEstadoCierre.DataBind();
        }
        else 
        {
            laccion = "CIERREXSUC";
    ...
    protected void btnEstadoSuc_Click(object sender, EventArgs e)
    {
        ...
        grvEstadoSuc.DataSource = dc.sp_ctrlCierresxSuc(laccion, ltipoSuc, lsucursal, lfechaInicio, lfechaFin);
    ...
    protected void uno()
    {
        ...
        using (StringWriter sw = new StringWriter())
        {
            ...
            //To Export all pages
            grvEstadoCierre.AllowPaging = false;
            /// this.BindGrid();

            grvEstadoCierre.HeaderRow.BackColor = Color.White;
            foreach (TableCell cell in grvEstadoCierre.HeaderRow.Cells)
            {
                cell.BackColor = grvEstadoCierre.HeaderStyle.BackColor;
            }
            foreach (GridViewRow row in grvEstadoCierre.Rows)
            {
                ...
                foreach (TableCell cell in row.Cells)
                {
                    if (row.RowIndex % 2 == 0)
                    {
                        cell.BackColor = grvEstadoCierre.AlternatingRowStyle.BackColor;
                    }
                    else
                    {
                        cell.BackColor = grvEstadoCierre.RowStyle.BackColor;
                    }
                    cell.CssClass = "textmode";
                }
            }

            grvEstadoCierre.RenderControl(hw);
